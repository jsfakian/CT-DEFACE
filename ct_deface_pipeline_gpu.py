...
For each DICOM case directory under dicom-root-in:
    ...
    NIfTI (single *_0000.nii.gz per case, nnUNet style)
        ↓
    CT-DEFACE (GPU, via run_CT-DEFACE.py --device cuda)
        ↓
...
- Use --gpu-id to pin inference to a specific GPU (default: 0).
"""
...
# -------------------------------------------------------------------------
# Run CT-DEFACE (GPU) and collect outputs
# -------------------------------------------------------------------------

def run_ct_deface_gpu(nifti_in_dir: str, nifti_out_dir: str,
                      gpu_id: str = "0", extra_args=None) -> List[str]:
    """
    Call run_CT-DEFACE.py with GPU settings.

    - Cleans nifti_out_dir before running.
    - Sets CUDA_VISIBLE_DEVICES to gpu_id.
    ...
    print(f"[CT-DEFACE] Running on GPU {gpu_id}: {' '.join(cmd)}")
...
def process_case(case_dicom_dir: str,
    ...
    # 2) CT-DEFACE (GPU)
    print(f"[step 2] CT-DEFACE (GPU {gpu_id})")
    run_ct_deface_gpu(nifti_in_dir, nifti_out_dir, gpu_id=gpu_id, extra_args=extra_args)
...
def main():
    ap = argparse.ArgumentParser(
        description="Batch CT-DEFACE pipeline (GPU): DICOM dirs -> defaced NIfTI + DICOM (full header reuse)."
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--gpu-id", default="0",
        help="CUDA device index to use for inference (default: 0). "
             "Set to a comma-separated list (e.g. '0,1') to expose multiple GPUs."
    )
    ap.add_argument(
        "--ct-extra-args", nargs=argparse.REMAINDER,
        help="Extra args passed to run_CT-DEFACE.py after -i/-o/--device."
    )
    ...
    for case_dir in cases:
        process_case(
            case_dir, root_in, root_out_dicom, root_out_nifti, work_root,
            gpu_id=args.gpu_id, extra_args=extra_args,
        )
